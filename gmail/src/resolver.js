...
async function getAccessToken() {
    ...
    try {
        ...
        console.log(`GMAIL RESOLVER: Successfully obtained access token, expires in ${tokenData.expires_in} seconds`);
        return accessToken;
...
const makeRequest = async (endpoint, options = {}) => {
    ...
    console.log(`GMAIL RESOLVER: making http request ${options.method} ${url} with options ${JSON.stringify(options)}`)
    ...
    const timeoutId = setTimeout(() => {
        console.error(`GMAIL RESOLVER: Request timeout after ${timeoutMs}ms - ${url} - ${JSON.stringify(options)}`);
        controller.abort();
    }, timeoutMs);
    ...
    try {
        ...
        console.log(`GMAIL RESOLVER: response ${response.status} ${response.ok}`, body)
    
        clearTimeout(timeoutId);

        if (!response.ok) {
            console.error(`GMAIL RESOLVER: HTTP Error ${response.status} - ${url} - ${JSON.stringify(options)}`);
        ...
        if (error.name === 'AbortError') {
            ...
            console.error(`GMAIL RESOLVER: Network unreachable (${error.code}) - ${url} - ${JSON.stringify(options)}`);
        } else if (error.code === 'ECONNRESET' || error.code === 'ETIMEDOUT') {
            console.error(`GMAIL RESOLVER: Connection error (${error.code}) - ${url} - ${JSON.stringify(options)}`);
        } else {
            console.error(`GMAIL RESOLVER: Request failed (${error.name}) - ${url} - ${JSON.stringify(options)}`);
        }
...
export const queryEmail = async (env, attrs) => {
    ...
    try {
        ...
        if (id) {
            ...
            const headers = messageDetail.payload?.headers?.reduce((acc, current) => {
                return {
                    ...acc,
...
export const updateEmail = async (env, attributes, newAttrs) => {
    ...
    try {
        ...
        if (Object.keys(data).length === 0) {
            return {"result": "error", "message": "No valid update operations provided"};
        }
...
export const createLabel = async (env, attributes) => {
    const data = {
        ...
        color: attributes.attributes.get('color') ? JSON.parse(attributes.attributes.get('color')) : undefined
    };
...
export const queryAttachments = async (env, attrs) => {
    ...
    if (!messageId || !attachmentId) {
        return {"result": "error", "message": "Message ID and Attachment ID are required"};
    }

    try {
        const result = await makeGetRequest(`/gmail/v1/users/me/messages/${messageId}/attachments/${attachmentId}`);
        return asInstance({
            data: result.data,
            size: result.size
        }, 'Attachments');
...
export const fetchAttachment = async (env, attributes) => {
    ...
    if (!threadId || !attachmentId) {
        return {"result": "error", "message": "Thread ID and Attachment ID are required"};
    }
